    ...
    var start = function () {
      ...
      for (var i = 0; i < 52; i++) {
        ...
        for (var j = 0; j < 7; j++) {
          ...
          item_html += '<rect class="day" width="11" height="11" style="stroke-width: 1; stroke: rgb(0,0,0,.325)" y="' + (y + 10) + '" fill="' + color + '" data-count="' + count + '" data-date="' + data_date + '"/>';
        }
      ...
      for (var i = 0; i < month_position.length; i++) {
        ...
        loop_html += '<text x="' + (item.x + 18) + '" y="-5" class="month">' + month_name + '</text>';
      }

      //Add Monday, Wenesday, Friday label
      loop_html += '<text text-anchor="middle" class="wday" dx="-10" dy="21">{0}</text>'.formatString(settings.h_days[0]) +
        '<text text-anchor="middle" class="wday" dx="-10" dy="39.8">{0}</text>'.formatString(settings.h_days[1]) +
        '<text text-anchor="middle" class="wday" dx="-10" dy="59.6">{0}</text>'.formatString(settings.h_days[2]) +
        '<text text-anchor="middle" class="wday" dx="-10" dy="79.4">{0}</text>'.formatString(settings.h_days[3]) +
        '<text text-anchor="middle" class="wday" dx="-10" dy="99.2">{0}</text>'.formatString(settings.h_days[4]) +
        '<text text-anchor="middle" class="wday" dx="-10" dy="119">{0}</text>'.formatString(settings.h_days[5]) +
        '<text text-anchor="middle" class="wday" dx="-10" dy="138.8">{0}</text>'.formatString(settings.h_days[6]);

      //Fixed size for now with width= 721 and height = 110
      var wire_html =
        '<svg width="100%" height="80%" viewBox="0 0 1200 160"  class="js-calendar-graph-svg">' +
...
*/
    ...
    var settings = $.extend({
      ...
      colors: ['#f1f3f4', '#d6e685', '#8cc665', '#44a340', '#44a340'],
      start_date: null,
      //List of name months
      month_names: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
      h_days: ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'],
